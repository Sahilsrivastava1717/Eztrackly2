...
export function WrapUpModal({ open, dueTasks = [], standupPriorities = [], onToggleTask, onSubmit, onSkip }) {
  ...
  const renderTaskRow = (t) => (
    <label key={t.id} style={{
      ...
      <input
        ...
        style={{ marginTop: 3, width: 15, height: 15, accentColor: "#2563eb", cursor: "pointer", flexShrink: 0 }}
      />
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#f8fafc", borderRadius: 18, padding: "26px 24px 22px",
            ...
            {completed.map((val, idx) => (
              <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <input
                  ...
                  style={{ flex: 1, padding: "10px 12px", borderRadius: 10, border: "1.5px solid #dbd6d6", placeholder: { color: "#040404", opacity: 1 }, fontSize: 14, outline: "none" }}
                />
                {completed.length > 1 && (
